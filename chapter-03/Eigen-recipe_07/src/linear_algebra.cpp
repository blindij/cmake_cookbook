...
//
// An example using the Eigen library. The program
// allocates a random square matrix and vector
// of dimension given at the command line.
// It then solve for Ax=b using LU decomposition.
#include <chrono>
#include <cmath>
#include <cstdlib>
#include <iomanip>
...
int main(int argc, char **argv){
   if (argc != 2) {
      std::cout << "Usage: ./.linear_algebra dim" << std::endl;
      return EXIT_FAILURE;
   }
   ...
   std::cout << "Scaling done, A and b save "
             << std::put_time(std::localtime(&end_time), "%a %b %d %Y %r\n")
             << "elapsed time: "
             << elapsed_seconds.count()
             << "s" << std::endl;
   ...
   std::cout << "Linear system solver done "
             << std::put_time(std::localtime(&end_time), "%a %b %d %Y %r\n")
             << "elapsed_time: " << elapsed_seconds.count() << "s\n";
